...
log "⚙️  Configuration service K3s..."
...
if [ -d "$K3S_DATA_DIR/server" ]; then
    ...
    warn "⚠️ Data-dir peut-être pas encore initialisé, on continue..."
fi
...
wait_for_deployment() {
    ...
    if kubectl wait --for=condition=available --timeout=${timeout}s deployment/$deployment -n $namespace 2>/dev/null; then
        ...
        warn "⚠️ $deployment timeout ou erreur - on continue"
        return 1
    fi
}
...
log "🗄️  Déploiement bases de données..."
...
log "🎙️  Déploiement APIs vocales..."
...
log "🖥️  Déploiement interface utilisateur..."
...
if curl -s -m 10 http://localhost:30100 >/dev/null 2>&1; then
    ...
    warn "⚠️ Interface web pas encore prête"
fi
