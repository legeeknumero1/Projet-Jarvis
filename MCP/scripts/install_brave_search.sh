...
if grep -q "BRAVE_API_KEY.*=.*[a-zA-Z0-9]" "$ENV_FILE"; then
    ...
    warn "⚠️  BRAVE_API_KEY manquante - ajout dans .env..."
    ...
    if [[ -f "api-key" ]]; then
        ...
        if [[ -n "$BRAVE_KEY_1" ]]; then
            ...
            warn "⚠️  Clés Brave non trouvées dans api-key"
        fi
    else
        warn "⚠️  Fichier api-key non trouvé"
    fi
fi
...
exit(0 if success else 1)
    " && log "✅ Brave Search MCP fonctionne correctement" || warn "⚠️  Test échoué - vérifiez les clés API"
    
else
    warn "⚠️  Impossible de tester - clé API manquante"
fi
...
if [[ -f "MCP/mcp_manager.py" ]]; then
    ...
    if ! grep -q "brave-search" MCP/mcp_manager.py; then
        ...
        warn "⚠️  Ajout manuel requis dans mcp_manager.py"
    else
        log "✅ Brave Search déjà configuré dans MCP Manager"
    fi
else
    warn "⚠️  MCP Manager non trouvé"
fi
...
echo ""
echo -e "${BLUE}🛡️ Fonctionnalités:${NC}"
...
echo ""
echo -e "${YELLOW}⚠️  Pour activer complètement:${NC}"
